...
if($ccuser->authlevel != 2){
    ...
    //create and output quick links
    	...
    	}else{ //proceed if the image was found
        	if(getSlug(4) == "confirmed"){ //delete the image if confirmed
        	
        	    if(file_exists('../uploads/' . $thisimage['thumbname'])){
                    unlink('../uploads/' . $thisimage['thumbname']);
        	    }
        		        
                if(file_exists('../uploads/' . $thisimage['imgname'])){
    	...
    	if($_SERVER['REQUEST_METHOD'] === 'POST'){
        	if($_POST['image-finalfile'] != ""){
        		...
        		if($stmt->rowCount() > 0){
        			...
        			<?php
        		}else{ //output error message if failed
        			?>
